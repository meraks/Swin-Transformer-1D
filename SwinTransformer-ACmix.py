...
# -*- coding: utf-8 -*-
'''
@File    :   SwinTransformer-ACmix.py
@Time    :   2022/08/12 12:09:03
@Author  :   HUANG Dong
@Version :   1.0
...
class WindowAttention1D_ACmix(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    Args:
        dim (int): Number of input channels.
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.fc = nn.Conv2d(3 * self.num_heads, 3, kernel_size=1, bias=True)
        # group convolution layer in Fig.3
        self.dep_conv = nn.Conv1d(3 * dim // self.num_heads, dim, kernel_size=3, bias=True,
    ...
    def reset_parameters(self):
        ...
        for i in range(3):
            kernel[i, i % 3] = 1.
        kernel = kernel.squeeze(0).repeat(self.dim, 1, 1)
        self.dep_conv.weight = nn.Parameter(data=kernel, requires_grad=True)
        # self.dep_conv.bias = zeros(self.dep_conv.bias)
        zeros(self.dep_conv.bias)

    def forward(self, x, L, mask=None):
        """
        Args:
            x: input features with shape of (B, L, C)
            mask: (0/-inf) mask with shape of (num_windows, Wl) or None
        """

        qkv = self.qkv(x)

        # fully connected layer
        f_all = qkv.reshape(x.shape[0], L, 3 * self.num_heads, -1).permute(0, 2, 1, 3)  # B, 3*nhead, L, C//nhead
        f_conv = self.fc(f_all).permute(0, 3, 1, 2).reshape(x.shape[0], 3 * x.shape[-1] // self.num_heads, L)  # B, 3*C//nhead, L
        # group conovlution
        out_conv = self.dep_conv(f_conv).permute(0, 2, 1)  # B, L, C

        # partition windows
        qkv = window_partition(qkv, self.window_size)  # nW*B, window_size, C

        B_, _, C = qkv.shape

        qkv = qkv.view(-1, self.window_size, C)  # nW*B, window_size, C
        ...
        x = window_reverse(x, self.window_size, L)  # B L C
...
class SwinTransformerBlock1D_ACmix(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention1D_ACmix(
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x, Lp, mask=attn_mask)  # B*nW, Wl, C
        # merge windows
...
class BasicLayer_ACmix(nn.Module):
...
if __name__ == "__main__":
    net = SwinTransformer1D(in_chans=32,
                 embed_dim=128,
                 depths=[2, 2, 6, 2],
                 num_heads=[4, 8, 16, 32],
                 window_size=7,
