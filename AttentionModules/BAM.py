...
class ChannelAttention1D(nn.Module):
    ...
    def forward(self, x):
        res = self.avgpool(x)
        res = self.ca(res)
        res = res.unsqueeze(-1).expand_as(x)
        return res
...
class SpatialAttention1D(nn.Module):
    def __init__(self, channel, reduction=16, num_layers=3, dia_val=2):
        ...
        self.sa.add_module('conv_reduce1',
                           nn.Conv1d(kernel_size=1, in_channels=channel, out_channels=channel // reduction))
        self.sa.add_module('bn_reduce1', nn.BatchNorm1d(channel // reduction))
        self.sa.add_module('relu_reduce1', nn.ReLU())
        for i in range(num_layers):
            self.sa.add_module('conv_%d' % i, nn.Conv1d(kernel_size=3, in_channels=channel // reduction,
                                                        out_channels=channel // reduction, padding=1, dilation=dia_val))
            self.sa.add_module('bn_%d' % i, nn.BatchNorm1d(channel // reduction))
            self.sa.add_module('relu_%d' % i, nn.ReLU())
